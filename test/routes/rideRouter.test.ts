...
test("Deve criar um passageiro e iniciar uma corrida", async function () {
	...
	expect(responseStartRide.status).toBe(201);
	...
	expect(outputGetRide.passenger_id).toBe(inputStartRide.passengerId);
	expect(outputGetRide.driver_id).toBe(null);
});
...
test("Deve criar um motorista e tentar iniciar uma corrida", async function () {
	...
	const responseStartRide = await axios.post(URL_RIDE, inputStartRide);
	expect(responseStartRide.data.error).toBe("Deve ser passageiro");
	expect(responseStartRide.status).toBe(500);
});
